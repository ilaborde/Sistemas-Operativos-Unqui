...
class TestsProgramLoader(unittest.TestCase):
    ...
    def test_when_have_a_program_Then_I_want_to_create_a_pcb_with_pid_1(self):
        ...
        program= Program()
        self.assertEqual(self.programLoader.createPcb(program).pid, 1)

    def test_when_have_a_program_Then_the_memory_is_write_with_a_new_program(self):

        program= Program()
        program.add(Instruction("Cpu Instruction1", InstructionType.cpu, ResourceType.Monitor))
        self.programLoader.createPcb(program)

        self.assertEqual(len(self.memory.cells), 1)
